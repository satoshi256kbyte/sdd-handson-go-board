...
要件とは「やりたいこと」ではありますが、
AI駆動開発・仕様駆動開発における要件は
*「売上を○○%あげたい」*
というレベルではありません。

少なくとも
*「売上を○○%あげるために、△△の効率をあげるため、□□を自動化する」*
ぐらいの粒度が必要です。

実案件では
*「□□を自動化するためのXX機能を作る」*
レベルまで分解してAIに投入するのが現実的です。

これぐらいでないと出来上がるものが大きすぎて人の方がレビューしきれなくなります。

---

## 要件定義の粒度はツールにも現れている

仕様駆動開発のツールにも、この傾向は現れています。
GitHub Spec Kitの場合は、
プロジェクトにおける要件定義フェーズで定めた各要件ごとに開発を回すイメージとなっています。

AWS AI-DLCの場合、一般的な要件定義をAIと一緒にやるような流れになっていますが、
「一緒にやる」でありざっくりしたものを入れればいい感じにやってくれる… 
というイメージではありません。

また、「一緒にやる」は開発者以外も含む関係者で画面を見ながらAIとやりとりするプロセスです。
ざっくりとした要件だとかなり時間がかかります。
このことからもAI駆動開発・仕様駆動開発における要件定義は「かなり小さい単位の要件」
でないと回らないと捉えた方がよいでしょう。
